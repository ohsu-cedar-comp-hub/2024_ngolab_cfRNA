...
list_pca_plot <- snakemake@output[['list_pca']]
cat(sprintf(c('PCA plot: ',list_pca_plot)))

ERCC_pca_plot <- snakemake@output[['ERCC_pca']]
...
list_sd_mean_plot <- snakemake@output[['list_sd_mean_plot']]
cat(sprintf(c('SD Mean plot: ',list_sd_mean_plot,'\n')))

list_distance_plot <- snakemake@output[['list_distance_plot']]
cat(sprintf(c('Distance plot: ',list_distance_plot,'\n')))

list_heatmap_plot <- snakemake@output[['list_heatmap_plot']]
cat(sprintf(c('Heatmap Plot: ', list_heatmap_plot, '\n')))

list_rds_out <- snakemake@output[['list_rds']]
cat(sprintf(c('RDS Output: ', list_rds_out, '\n')))

list_rld_out <- snakemake@output[['list_rld_out']]
cat(sprintf(c('RLD Output: ', list_rld_out, '\n')))


ERCC_sd_mean_plot <- snakemake@output[['ERCC_sd_mean_plot']]
cat(sprintf(c('SD Mean plot: ',ERCC_sd_mean_plot,'\n')))

ERCC_distance_plot <- snakemake@output[['ERCC_distance_plot']]
cat(sprintf(c('Distance plot: ',distance_plot,'\n')))

ERCC_heatmap_plot <- snakemake@output[['ERCC_heatmap_plot']]
cat(sprintf(c('Heatmap Plot: ', heatmap_plot, '\n')))

ERCC_rds_out <- snakemake@output[['ERCC_rds']]
cat(sprintf(c('RDS Output: ', ERCC_rds_out, '\n')))

ERCC_rld_out <- snakemake@output[['ERCC_rld_out']]
cat(sprintf(c('RLD Output: ', ERCC_rld_out, '\n')))
...
cts <- read.table(counts, row.names=1)
counts_ens <- rownames(cts)
rownames(cts) <- gsub("\\..*","",counts_ens)
...
saveRDS(dds, file=rds_out, version =2)
...
# Pairwise PCA Plot with more than one PCA parameter
if (length(labels)>1) {
  pca_plot2 <- sub("$","twoDimensional_pca_plot_list.pdf", Dir)
  pcaData <- plotPCA(rld, intgroup=c(labels[[1]], labels[[2]]), returnData=TRUE)
  pdf(pca_plot2, 5, 5)
  percentVar <- round(100 * attr(pcaData, "percentVar"))
  ggplot(pcaData, aes_string("PC1", "PC2", color=labels[[1]], shape=labels[[2]])) +
...
# SD mean plot
pdf(list_sd_mean_plot)
meanSdPlot(assay(rld))
dev.off()

# Heatmap of distances
pdf(list_distance_plot)
sampleDists <- dist(t(assay(rld)))
sampleDistMatrix <- as.matrix(sampleDists)
...
if (length(group)>0) {
  ...
  # Pairwise PCA Plot
  pdf(sub("$", "subsetted_pca_plot_list.pdf", Dir), 5, 5)
  plotPCA(rld, intgroup=labels[[1]])
  dev.off()
  # Pairwise PCA Plot with more than one PCA parameter
  if (length(labels)>1) {
    pcaData <- plotPCA(rld, intgroup=c(labels[[1]], labels[[2]]), returnData=TRUE)
    pdf(sub("$", "subsetted_twoDimensional_pca_plot_list.pdf", Dir), 5, 5)
    percentVar <- round(100 * attr(pcaData, "percentVar"))
    ggplot(pcaData, aes_string("PC1", "PC2", color=labels[[1]], shape=labels[[2]])) +
      geom_point(size=3) +
      xlab(paste0("PC1: ",percentVar[1],"% variance")) +
  ...
  pdf(sub("$", "subsetted_heatmap_list.pdf", Dir), 5, 5)
  pheatmap(assay(rld)[topGenes,], cluster_rows=T, scale="row", fontsize=6,fontsize_row=6,fontsize_col=6,show_rownames=T, cluster_cols=T, annotation_col=annot, labels_col=as.character(rownames(df)), main = paste("Heatmap of top 50 DE genes across selected samples"))
  dev.off()
}
...
# Pairwise PCA Plot with more than one PCA parameter
if (length(labels)>1) {
  pca_plot2 <- sub("$","twoDimensional_pca_plot_ERCC.pdf", Dir)
  pcaData <- plotPCA(rld, intgroup=c(labels[[1]], labels[[2]]), returnData=TRUE)
  pdf(pca_plot2, 5, 5)
  percentVar <- round(100 * attr(pcaData, "percentVar"))
  ggplot(pcaData, aes_string("PC1", "PC2", color=labels[[1]], shape=labels[[2]])) +
...
# SD mean plot
pdf(ERCC_sd_mean_plot)
meanSdPlot(assay(rld))
dev.off()

# Heatmap of distances
pdf(ERCC_distance_plot)
sampleDists <- dist(t(assay(rld)))
sampleDistMatrix <- as.matrix(sampleDists)
...
saveRDS(ERCC_dds, file=rds_out, version = 2)
saveRDS(ERCC_rld, file=rld_out, version = 2)
...
if (length(group)>0) {
  ...
  # Pairwise PCA Plot
  pdf(sub("$", "subsetted_pca_plot_ERCC.pdf", Dir), 5, 5)
  plotPCA(rld, intgroup=labels[[1]])
  dev.off()
  # Pairwise PCA Plot with more than one PCA parameter
  if (length(labels)>1) {
    pcaData <- plotPCA(rld, intgroup=c(labels[[1]], labels[[2]]), returnData=TRUE)
    pdf(sub("$", "subsetted_twoDimensional_pca_plot_ERCC.pdf", Dir), 5, 5)
    percentVar <- round(100 * attr(pcaData, "percentVar"))
    ggplot(pcaData, aes_string("PC1", "PC2", color=labels[[1]], shape=labels[[2]])) +
      geom_point(size=3) +
      xlab(paste0("PC1: ",percentVar[1],"% variance")) +
  ...
  pdf(sub("$", "subsetted_heatmap_ERCC.pdf", Dir), 5, 5)
  pheatmap(assay(rld)[topGenes,], cluster_rows=T, scale="row", fontsize=6,fontsize_row=6,fontsize_col=6,show_rownames=T, cluster_cols=T, annotation_col=annot, labels_col=as.character(rownames(df)), main = paste("Heatmap of top 50 DE genes across selected samples"))
  dev.off()
}
